...
(function() {
  ...
  if (window.__YD_FOOTER_V3_60__) {
    return;
  }
  window.__YD_FOOTER_V3_60__ = true;
  ...
  const ydStatus = { version: '3.60', page: location.pathname, features: {} };
  ...
  function bindOptionFlow() {
    ...
    function sizeTabs(s) {
      ...
      /* 1251(스레드 친구 전용): 중량+가격 고정 표기 (소유자 지시 2026-08-11) — 다른 상품은 기존 표기 유지 */
      if (flowIdx === '1251') {
        return '<div class="yd-bs-category-grid" role="group" aria-label="라인 선택">' + btn('S', '325g', '4,990원') + btn('L', '420g', '5,190원') + btn('P', 'PREMIUM', '프리미엄 도시락') + '</div>';
      }
    ...
    window.setTimeout(function() {
      Object.keys(ydStatus.features).forEach(function(key) {
        if (!ydStatus.features[key].ok) {
          console.warn('[YD v3.60] 미적용 감지: ' + key + ' — ' + ydStatus.features[key].note + ' (YD_CHECK()로 상세 확인)');
        }
      });
    }, 6000);
  });
